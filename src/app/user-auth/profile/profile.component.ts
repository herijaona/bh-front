...
export class ProfileComponent implements OnInit {
  ...
  ngOnInit() {    
    this.getProfile().then(()=>{
      this.EditForm.setValue({bh_lastname: this.details.lastname, bh_firstname: this.details.firstname, bh_functions : this.details.function});      
    });
  }
  getProfile(){
    return new Promise((
      ...
      this.auth.profile().toPromise().then(user => {
        console.log(user);
