	...
	async submitSstr() {
		try {
			if (this.toDoAct == this.newAct) {
				...
				let resp: any = await this.cs.sstrFrontSaveData(
					this.idVidYouTube
				);
				if (resp.status == "OK") {
					this.successAction();
			...
			if (this.toDoAct == this.editAct) {
				if (
					...
					this.sstrForm.value.sstrVideoUrl !=
						this.sstrData.video_url ||
					this.idVidYouTube.id_video != this.sstrData.id_video
				) {
					this.idVidYouTube[
						"caption"
					] = this.sstrForm.value.sstrCaption;
					this.idVidYouTube[
						"video_url"
					] = this.sstrForm.value.video_url;

					let sstrUpdate: any = await this.cs.updatesstrData({
